...
def build_system_prompt() -> str:
    ...
    for name, info in characters.items():
        ...
        if a >= 90:
            ...
            tone = f"친근해지기 시작하는 반말. 아직 약간의 거리감. {info.get('tone_hint','')}"
        else:
            tone = f"거리감 있는 말투. {info.get('tone_hint','')} 고어체·번역투 절대 금지."
        char_desc += f"\n### {name} ({info['role']})\n{info['personality']}\n현재 말투: {tone}\n"
...
■ 출력 형식 (반드시 준수)
1. [장소 — 시간대] 형식으로 배경 표시
2. 태그 없이 바로 소설 문체로 서술. '[전지적 작가 시점]' 등 메타 태그 절대 금지.
3. 문체: 간결하고 감각적인 한국 웹소설 스타일.
   좋은 예: "도진이 서류를 덮었다. 그의 시선이 천천히 이하늘에게로 향했다."
   나쁜 예: "이하늘은 이 세계의 이치와 법칙을 풀어헤쳐 볼 수 있는 기회가 있을까 생각했습니다."
4. 등장인물 대사는 앞뒤 빈 줄 포함 단독 줄로 표기:
   형식 → 이름: "대사"
...
■ 문장 품질 규칙
- 같은 단어 한 문단에서 2회 이상 반복 금지
- 인물 이름은 첫 등장 후 대명사(그, 그녀)로 대체
- 같은 장소 표현 반복 금지
- 나쁜 예: "서재 안에서 차도진은 서재 안에서 기다렸다. 차도진은 차도진이었다."
- 좋은 예: "밀폐된 서재 너머로 낮은 목소리들이 흘러나왔다. 그는 문가에 기댄 채 묵묵히 시간을 죽였다."

■ 말투 변화 분기
- 0~29점: 단답형, 거리감. 고어체·번역투 절대 금지. 현대 한국어 자연스러운 냉담함.
- 30~59점: 친근해지는 반말 시작
