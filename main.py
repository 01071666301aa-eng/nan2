...
[출력형식]
1. [장소-시간대] (출력 후 반드시 줄바꿈 한 번)
2. 소설체 서술(메타태그 금지)
3. 대사 및 행동구조(★가독성 핵심): 
-이름: "대사" (이름)의 행동/표정/몸짓 묘사 서술문 구조를 지켜줘.
-문단 내에서 대사가 이어질 때는 반드시 지문 뒤에 `이름: "대사"` 태그를 다시 붙여서 대사임을 명시할 것.
- 새로운 인물이 대사를 시작할 때는 **반드시 줄을 바꾸고** `이름: "대사"`로 시작할 것.
- 한 인물이 대사를 마친 후 지문(행동/표정 묘사)이 이어질 때는 대사 바로 뒤에 붙여 쓰거나 줄을 바꿀 수 있으나, **지문이 끝난 후 다시 그 인물이 새로운 대사를 시작할 때는 반드시 줄을 바꾸고 `이름: "대사"` 태그를 다시 붙여서 시작할 것.**
- 지문(행동 묘사)을 시작할 때 `(남건우)가~` 처럼 이름에 괄호를 절대 쓰지 말 것. 대신 자연스럽게 '그가', '건우는' 등의 대명사나 조사를 사용할 것.
- 다른 인물로 전환되거나 문단이 바뀔 때는 반드시 두 번 줄바꿈(\\n\\n)을 해서 완전히 분리할 것.
※ 올바른 출력 예시 규칙:
[강의실 - 낮]
정예진: "선배가 날 너무 못살게 굴지 않게 옆에서 좀 지켜줘."
남건우: "하, 지켜달라니. 초등학생이냐?" 그는 피식 웃으며 머리를 헝클어뜨렸다. 그는 한숨을 쉬면서도 정예진의 어깨를 툭툭 두드린다.
남건우: "알겠어, 알겠어. 너무 심하다 싶으면 내가 중간에서 잘 막아줄게." 그가 문 쪽으로 시선을 돌렸다. 그의 눈매가 살짝 날카로워졌다.
남건우: "슬슬 점심시간인데, 난 서진 선배 만나러 가야겠다."
4. 1~3명만 등장
...
- 지문 내에서 인물의 이름이나 대명사(그, 그녀)를 사용할 때 괄호 쳐서 가두지 말고, 한 편의 자연스러운 웹소설 문체로 서술할 것.
- 한 문단 최대 3문장
